...
export async function propagateDates(config: any) {
    ...
    config['course-website'].schedule.forEach((week: any) => {
...
// That is overridden by a pre-existing draft value that exists in _quarto.yml
...
export async function writeSchedule(config: any, tempFilesDir: string) {
    ...
    await Deno.writeTextFile(outputPath, stringify(config['course-website'].schedule));
...
export async function writeAutoNav(config: any, tempFilesDir: string) {
    ...
    if (!config['course-website'].hasOwnProperty('auto-nav')) {
        return; 
    }
    ...
    if (config['course-website']['auto-nav'].hasOwnProperty('sidebar')) {
        ...
        sidebarTypes.forEach(sidebarType => {
            ...
            config['course-website'].schedule.forEach(week => {
...
export async function writeAutoListings(config: any, tempFilesDir: string, offset: string ) {
    ...
    for (const week of config['course-website'].schedule) {
...
export async function removeTempDir(config: any, tempFilesDir: string) {
  ...
  if (!config['course-website'].debug) {
    try {
      await Deno.remove(tempFilesDir, { recursive: true });
